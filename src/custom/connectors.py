...
class SPARQLConnector(Connector):
    ...
    def fetch(self, query: str) -> Response:
        ...
        """
        url = f"{self.endpoint}/query?query={parse.quote(query)}" if 'wikidata' not in self.endpoint \
            else f"{self.endpoint}?query={parse.quote(query)}"

        if 'wikidata' in self.endpoint:
            self._headers['User-Agent'] = 'jose-test embeddings extractor bot'
        ...
        with requests.get(url, headers=self._headers) as res:
            try:
                ...
                new_content_str = content_str.replace('\\U', '\\u')
                json_dict = json.loads(new_content_str)
                # f = open("json_responses.txt", "wb")
                # f.write(res.content)
                # f.close()
            return json_dict
